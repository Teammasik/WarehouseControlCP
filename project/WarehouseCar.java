...
public class WarehouseCar extends TemplateWarehouse{
    String name = "Склад автомобилей";
    ...
    private ArrayList<Item> listOfItems = new ArrayList<>(List.of(
            (new Item("2JZ-GTE", 0, 0, 0, 30, "деталь")),
            (new Item("Coilovers",  0, 0, 0, 10, "деталь")),
            (new Item("Oil",  0, 0, 0, 3, "деталь")),
            (new Item("Tires",  0, 0, 0, 15, "деталь")),
            (new Item("Bumper",  0, 0, 0, 10, "деталь"))
    ));
    @Override
    ...
    int toUpgrade(int money){
        int price = 10000 * level * level; // to place 100000
        if (level < 3) {
            if (money >= price) {
                money -= price;
                level += 1;
    ...
    boolean recieveItems(ArrayList<Item> itemArray){
        ...
        for (int i = 0; i < itemArray.size(); i++) {
            if (itemArray.get(i).type.equals("деталь")) {
